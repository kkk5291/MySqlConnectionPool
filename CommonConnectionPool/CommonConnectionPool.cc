...
void ConnectionPool::produceConnectionTask()
{
	for (;;)
	{
		...
		{
			cv.wait(lock);	//队列不为空，因此生产线程进入等待状态
		}
...
std::shared_ptr<Connection> ConnectionPool::getConnection()
{
	...
	connection的析构函数，connection就被close掉了，这里需要自定义shared_ptr的
	释放资源的方式，把connection直接归还到queue当中
	*/

	std::shared_ptr<Connection> sp(_connectionQue.front(),
		[&](Connection* pcon) {
			//这里是在服务器应用线程中调用的。所以一点要考虑队列的线程安全操作
			std::unique_lock<std::mutex> lock(_queueMutex);
...
void ConnectionPool::scannerConnectionTask()
{
	for (;;)
	{
		...
		while (_connectionCnt > _initSize)
		{
			...
			{
				break;	//队头的连接没有超过_maxIdleTime,其他连接也不会
			}
		}
	}
}